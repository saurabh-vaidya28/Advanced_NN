...
What is Batch Normalization? Batch Normalization is a technique used to normalize the input of each layer, by adjusting and scaling activations so that they have a mean of zero and a standard deviation of one. This is done for each mini-batch of data during training, hence the name.
...
Step 3: Scale and shift the activations using learned parameters 
𝛾
γ and 
𝛽
β, allowing the model to restore the original distribution if needed.
